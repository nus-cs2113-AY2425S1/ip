...
public class Prompt {
    ...
    public static void splitCommandAndTask(String lineBufferString) throws EmptyArgumentException {
        ...
        if (lineBufferString.contains(" ")) {
            Prompt.command = lineBufferString.split(" ")[0];
            Prompt.task = lineBufferString.substring(Prompt.command.length() + 1);
        } else {
            Prompt.command = lineBufferString;
            Prompt.task = null;
        }
    }
}
